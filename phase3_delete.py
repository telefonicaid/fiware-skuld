...
if os.path.exists('users_trusted_ids.txt'):
    ...
    elif 'TRUSTEE_PASSWORD' in dir(settings.settings):
        password = settings.settings.TRUSTEE_PASSWORD
    else:
...
for line in lines:
    try:
        ...
        if use_trust_ids:
            ...
            logger.info('Obtaining resources of user {0} {3} ({1}/{2})'.format(
                user, count, total, user_id))
